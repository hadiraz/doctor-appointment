...
const ResponsiveNav = ({ className , setMenuStatus }: { setMenuStatus : React.Dispatch<React.SetStateAction<boolean>> , className?: string }) => {
  return (
    <section
      className={`w-[70vw] min-h-screen flex flex-col absolute top-0 right-0 bg-white rounded-tl-3xl rounded-bl-3xl transition-all duration-200 ease-in ${className}`}
    >
      ...
      <nav className="w-full h-full flex mt-3">
        <ul className="w-full h-full flex flex-col">
          <li className="flex items-center">
            ...
            <span className="flex capitalize text-primary font-semibold">
              home
            </span>
          </li>
        </ul>
