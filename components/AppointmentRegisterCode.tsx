    ...
    <div  style={{...glassStyle}} className={`flex w-fit flex-col items-center rounded-2xl text-text transition-all duration-200 p-5`}>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col justify-center items-center w-full">
        <div className="flex w-full justify-center items-center">
            <SplitedInputs setSubmitStatus={setSubmitStatus} inputsNumber={5} stringCode={stringCode} setStringCode={setStringCode} />
        </div>
        <div className="flex w-full justify-between items-center mt-6">
          <button className="w-fit text-sm font-bold mt-5"  type="button" onClick={()=>setSectionSelected(0)}>
              Edit number
          </button>
          <button type="submit" disabled={submitStatus} className="opacity-100 disabled:opacity-70 rounded-2xl font-bold px-5 py-2 bg-primary w-fit text-white mt-5 shadow-lg">
            Next
          </button>
        </div>
      </form>
